...
class MOZ_NONHEAP_CLASS Handle {
 public:
  ...
  template <typename S, typename = typename std::enable_if_t<
...
class JSRegExp : public HeapObject {
 public:
  JSRegExp() : HeapObject() {}
  JSRegExp(js::RegExpShared* re) {
    value_ = JS::PrivateGCThingValue(reinterpret_cast<js::gc::Cell*>(re));
  }
  ...
  // XXXMC Do we really need 2^16 captures? o.O
